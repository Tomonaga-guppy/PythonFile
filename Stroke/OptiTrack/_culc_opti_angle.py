...
def main():
    ...
    for i, csv_path in enumerate(csv_paths):
        ...
        for frame_num in full_range:
            #使う関節の選択
            rasi = keypoints_mocap[frame_num, 15, :]
            lasi = keypoints_mocap[frame_num, 4, :]
            rank = keypoints_mocap[frame_num, 13, :]
            lank = keypoints_mocap[frame_num, 2, :]
            rank2 = keypoints_mocap[frame_num, 14, :]
            lank2 = keypoints_mocap[frame_num, 3, :]
            rknee = keypoints_mocap[frame_num, 18, :]
            lknee = keypoints_mocap[frame_num, 6, :]
            rknee2 = keypoints_mocap[frame_num, 19, :]
            lknee2 = keypoints_mocap[frame_num, 7, :]
            rtoe = keypoints_mocap[frame_num, 24, :]
            ltoe = keypoints_mocap[frame_num, 12, :]
            rhee = keypoints_mocap[frame_num, 17, :]
            lhee = keypoints_mocap[frame_num, 5, :]
            ...
            y_rthigh = +(c * np.sin(beta) - d_leg/2)
            y_lthigh = -(c * np.sin(beta)- d_leg/2)
            ...
            hip = (rthigh + lthigh) / 2
            lumbar = (0.47 * (rasi + lasi) / 2 + 0.53 * (rank + lank) / 2) + 0.02 * np.array([0, 0, 1])
            ...
            euler_angles_pelvis = R.from_matrix(rot_pelvis).as_euler('YZX', degrees=True)
