...
target_camera = "fr"  #解析対象カメラ：fr か fl
...
mov_dir = root_dir / mov / "gopro" / target_camera / "ext_cali.MP4"  #キャリブレーション動画が入っているフォルダ
...
def main():
    ...
    cap = cv2.VideoCapture(str(mov_dir))
    ...
    # CameraParams_path = os.path.join(os.path.dirname(os.path.dirname(root_dir)), "int_cali", target_facility, f"Intrinsic_{target_camera}.pickle")
    CameraParams_path =root_dir / "int_cali" / target_facility / f"Intrinsic_{target_camera}.pickle"
    try:
    ...
    for iRet,rvec,tvec in zip(range(rets),rvecs,tvecs):
        ...
        extrinsicImageFolder = root_dir / mov / "gopro" / target_camera / "extrinsicCalib"
        extrinsicImageFolder.mkdir(exist_ok=True)
        ...
        savePath2 = root_dir / mov / "gopro" / target_camera / f"extrinsicCalib_soln{iRet}.jpg"
        cv2.imwrite(str(savePath2),imageCropped)
        ...
        saveExtPath = root_dir / mov / "gopro" / target_camera / f"cameraIntrinsicsExtrinsics_soln{iRet}.pickle"
        ...
        pointsSavePath = root_dir / mov / "gopro" / target_camera / "object3d_twodpoints.csv"
