...
def main():
    for itsv, tsv_file in enumerate(tsv_files):
        ...
        for _range in ranges:
            ...
            for frame_idx_in_range, original_frame_num in enumerate(_range):
                try:
                    ...
                    e_x_rfoot = (rtoe[frame_idx_in_range,:] - rhee[frame_idx_in_range,:]) / np.linalg.norm(rtoe[frame_idx_in_range,:] - rhee[frame_idx_in_range,:])
                    e_y0_rfoot = rank2[frame_idx_in_range,:] - rank[frame_idx_in_range,:]
        ...
        if 'L_Knee' in angle_df.columns:
            for frame in angle_df.index:
                angle_df.loc[frame, 'L_Knee'] = - angle_df.at[frame, 'L_Knee']
        if 'R_Ankle' in angle_df.columns:
            for frame in angle_df.index:
                angle_df.loc[frame, 'R_Ankle'] = 180 - angle_df.at[frame, 'R_Ankle']
        if 'L_Ankle' in angle_df.columns:
            for frame in angle_df.index:
                angle_df.loc[frame, 'L_Ankle'] = 180 - angle_df.at[frame, 'L_Ankle']
