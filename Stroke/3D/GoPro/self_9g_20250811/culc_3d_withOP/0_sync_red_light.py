...
def show_detection_frames(cap, detected_frame_num, roi_box, display_scale, total_frames, total_duration, video_path):
    ...
    output_filename = f"LED_detection_frame_{int(detected_frame_num)}_time_{detected_frame_num/fps:.2f}s.jpg"
    output_path = video_path.parent / output_filename
...
def main():
    ...
    for direction in ["fl", "fr", "sagi"]:
        VIDEO_PATH = Path(r"G:\gait_pattern\20250811_br\sub1\thera1-1" + r"\\" + direction + r"\1-1-1.MP4")
