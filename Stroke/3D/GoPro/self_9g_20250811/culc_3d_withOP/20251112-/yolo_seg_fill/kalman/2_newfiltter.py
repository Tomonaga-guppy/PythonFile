...
alpha_ankle_x, beta_ankle_x = 0, 0
alpha_ankle_y, beta_ankle_y = 0.1, 0.02
alpha_smalltoe_x, beta_smalltoe_x = 0.1, 0.05
alpha_smalltoe_y, beta_smalltoe_y = 0.1, 0.02
alpha_bigtoe_x, beta_bigtoe_x = 0.1, 0.05
alpha_bigtoe_y, beta_bigtoe_y = 0.1, 0.02
alpha_heel_x, beta_heel_x = 0.1, 0.05
alpha_heel_y, beta_heel_y = 0.1, 0.02
...
if display_coordinates:
    ...
    for joint_name, data in plot_data.items():
        alpha_x, beta_x = filter_params[f'{joint_name}_params'][0], filter_params[f'{joint_name}_params'][1]
        alpha_y, beta_y = filter_params[f'{joint_name}_params'][2], filter_params[f'{joint_name}_params'][3]
        ...
        plt.legend()
        plt.grid(True)
        plt.savefig(os.path.join(output_dir, f'coord_{joint_name}_x_a{alpha_x}_b{beta_x}.png'))
        plt.close()
        ...
        plt.legend()
        plt.grid(True)
        plt.savefig(os.path.join(output_dir, f'coord_{joint_name}_y_a{alpha_y}_b{beta_y}.png'))
        plt.close()
