...
def main():
    for condition in condition_list:
        ...
        #2d上でのキーポイントを取得"
        cd = ["Undistort_op", "Undistort_black_fill_black_op"]

        for cd_name in cd:
            openpose_dir = condition_dir / "sagi" / f"{cd_name}.json"
            if not openpose_dir.exists():
                print(f"    {openpose_dir}が存在しません。スキップします。")
                continue
            print(f"    {openpose_dir}を読み込みます。")
            ...
            angle_mean_std_df.to_csv(openpose_dir.with_name(f"joint_angle_mean_std_{condition}_{cd_name}.csv"))
