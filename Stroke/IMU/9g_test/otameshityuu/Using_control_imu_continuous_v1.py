...
def get_measurement_condition(root_dir, measurement_count):
    """
    計測条件を入力し、保存先ディレクトリを作成する関数
    measurement_count: 何回目の計測か
    """
    ...
    return imu_save_folder, sub_num, thera_num
...
def main(ports, port_dict, root_dir):
    ...
    while continue_measurement:
        ...
        if measurement_count == 1:
            imu_save_folder, sub_num, thera_num = get_measurement_condition(root_dir, measurement_count)
        else:
            current_date = datetime.now().strftime('%Y%m%d')
