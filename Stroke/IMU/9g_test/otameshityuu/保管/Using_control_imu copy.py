...
def set_device_time(ser):
    ...
    params += struct.pack('<H', int(now.microsecond / 1000))

    bcc = header ^ cmd
    for p in params:
        bcc ^= p

    command = bytearray([header, cmd]) + params + bytearray([bcc])

    ser.reset_input_buffer()
    ser.write(command)

    ack = bytearray()
    deadline = time.time() + 0.5  # 最大0.5秒待つ
    while len(ack) < 4 and time.time() < deadline:
        chunk = ser.read(4 - len(ack))
        if chunk:
            ack.extend(chunk)
        else:
            time.sleep(0.01)

    if len(ack) < 4 or ack[0] != header or ack[1] != 0x8F:
        print(f"時刻設定の応答を取得できませんでした {ser.port}: {ack.hex()}")
        return False

    expected_bcc = ack[0] ^ ack[1] ^ ack[2]
    if ack[3] != expected_bcc:
        print(f"時刻設定応答のチェックサムが一致しませんでした {ser.port}: {ack.hex()}")
        return False

    if ack[2] != 0x00:
        print(f"時刻設定がエラーを返しました {ser.port}: {ack[2]}")
        return False

    print(f"{ser.port} の時刻を同期しました。")
    return True
...
def get_entry_count(ser, port):
    ...
    response = ser.read(4)
    if len(response) < 4 or response[0] != header or response[1] != 0xB6:
...
def read_save_memory(port, save_path, ser=None, keep_open=False):
    """メモリからデータを取得しCSVに保存する。serが渡されればそれを使用する。"""

    local_ser = ser
    if local_ser is None:
        local_ser = serial.Serial()
        local_ser.port = port
        local_ser.timeout = 1.0
        local_ser.baudrate = 115200

        i = 1
        while not local_ser.is_open:
            try:
                local_ser.open()
            except Exception:
                time.sleep(1)
    ...
    while local_ser.in_waiting > 0:  # バッファ内のデータをクリア 重要！
        local_ser.readline()
        time.sleep(0.01)

    # 最新の計測記録を取得し、CSVに保存
    entry_count = get_entry_count(local_ser, port)
    if entry_count > 0:
        accel_gyro_data, geomagnetic_data, extension_data = read_entry(local_ser, entry_count, port)
        # CSVに保存
    ...
    # # 計測データの記録をクリア
    # if entry_count > 40:
    #     clear_measurement_data(local_ser)

    if not keep_open and local_ser.is_open:
        local_ser.close()
...
def open_serial_port(port):
    ...
    while not ser.is_open:
        if i == 10:
            raise RuntimeError(f"シリアルポートを開くことができませんでした。 ({port})")
        try:
            ser.open()
        except Exception:
            time.sleep(0.1)
...
def prepare_devices(serials, ports, initial_config_done, is_first_cycle):
    for port in ports:
        ser = serials[port]
        if set_device_time(ser):
            pass
        else:
            print(f"時刻設定の確認に失敗しました {port}")

        if is_first_cycle or not initial_config_done.get(port, False):
            flag = configure_accelgyro(ser, port)
            if not flag:
                raise RuntimeError(f"加速度の設定に失敗しました。 計測を終了します({port})")

            flag = configure_magnetic(ser, port)
            if not flag:
                raise RuntimeError(f"地磁気の設定に失敗しました。 計測を終了します({port})")
            ...
            initial_config_done[port] = True
        else:
            pass


def start_measurements_simultaneously(serials, ports):
    ...
    def worker(port):
        ...
        try:
            ...
            if not flag:
                raise RuntimeError(f"計測の開始に失敗しました。 計測を終了します({port})")
    ...
    if errors:
        # 先頭のエラーのみ報告
        port, exc = errors[0]
        raise RuntimeError(f"{port} の計測開始処理でエラーが発生しました: {exc}")

    return start_time_dict


def stop_all_measurements(serials, ports):
    for port in ports:
        ser = serials[port]
        try:
            set_voltage(ser, params=[9, 1, 0, 0])
            ...
            print(f"計測停止処理でエラーが発生しました ({port}): {exc}")


def perform_measurement_cycle(serials, ports, initial_config_done, is_first_cycle):
    stop_event.clear()
    prepare_devices(serials, ports, initial_config_done, is_first_cycle)

    try:
        start_time_dict = start_measurements_simultaneously(serials, ports)
    except Exception:
        stop_all_measurements(serials, ports)
        raise
...
def download_all_memory(serials, ports, port_dict, start_time_dict, save_dir):
    print("メモリの書き出しを5台分行います 少々お待ちください")
    start = time.time()

    for i, port in enumerate(ports, start=1):
        if port not in start_time_dict:
            print(f'{i}/{len(ports)} 計測開始時刻が不明なため、データを保存できませんでした {port}')
            continue

        save_path = save_dir / f'{port_dict[port]}_{start_time_dict[port]}.csv'
        flag = read_save_memory(port, save_path, ser=serials[port], keep_open=True)
        if flag:
            print(f'{i}/{len(ports)} 計測データを "{save_path}" に保存しました {port}')
        else:
            print(f'{i}/{len(ports)} 計測データは保存できませんでした {port}')
...
def close_all_serials(serials):
    for port, ser in serials.items():
        try:
            ...
            print(f"シリアルポートのクローズ時にエラーが発生しました ({port}): {exc}")


def ask_continue_measurement():
    answer = ""
...
if __name__ == "__main__":
    ...
    reuse_port_flag = "a"
    while reuse_port_flag not in ("y", "n"):
    ...
    serials = {port: open_serial_port(port) for port in ports}
    initial_config_done = {port: False for port in ports}
    is_first_cycle = True
    ...
    while continue_measurement:
        ...
        try:
            start_time_dict = perform_measurement_cycle(serials, ports, initial_config_done, is_first_cycle)
        except Exception as exc:
            print(f"計測準備または開始時にエラーが発生しました: {exc}")
            break

        download_all_memory(serials, ports, port_dict, start_time_dict, imu_save_folder)
        is_first_cycle = False
        stop_event.clear()
        continue_measurement = ask_continue_measurement()

    close_all_serials(serials)
    print("\n全ての計測が終了しました。ウィンドウを閉じて終了してください")