...
def OpenFace(root_dir):
    ...
    for i,RGB_dir in enumerate(RGB_dirs):
        ...
        while True:
            ...
            try:
                theta_camera =np.arccos(abs(accel[frame_count][1])/np.linalg.norm(accel[frame_count]))
            except IndexError:
                break
            ...
            # print(f"xpixel_scale = {xpixel_scale}")
            # print(f"ypixel_scale = {ypixel_scale}")
            seal_x = seal_x_pixel * xpixel_scale  #pixelとmmの原点差考慮してない
            ...
            try:
                for i in range(68):
                    ...
                    x = (float(OpenFace_result[frame_count][i+5]) - seal_x_pixel) * xpixel_scale + seal_x
                    y = (float(OpenFace_result[frame_count][i+73]) - seal_y_pixel) * ypixel_scale + seal_y
