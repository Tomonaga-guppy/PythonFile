...
def OpenFace(root_dir):
    pattern = os.path.join(root_dir, '*b1*/RGB_image')
    ...
    for i,RGB_dir in enumerate(RGB_dirs):
            ...
            try:
                for i in range(68):
                    if key == "scale_seal":
                        ...
                        x = float(OpenFace_result[frame_count][i+141]) * xpixel_scale
                        y = float(OpenFace_result[frame_count][i+209]) * ypixel_scale
        ...
        for i in range(ply_list_all.shape[0]):
            ...
            # PLYファイルに書き込む
            with open(dir_path + f"ply/random_clund{frame_count}.ply", "w") as ply_file:
            # with open(dir_path + f"random_clund.ply", "w") as ply_file:
            ...
            # PLYファイルに書き込む
            with open(dir_path + f"ply/face_clund{frame_count}.ply", "w") as ply_file:
