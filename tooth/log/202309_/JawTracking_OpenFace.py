...
import glob
import math
import subprocess #pythonでコマンドの実行
import csv
import cv2
import numpy as np
import time
...
def MovieAnalyse_8(root_dir):
    ...
    for i,video_file in enumerate(mp4_files):
        ...
        while True:
            ...
            value = []
            value = np.array(SealDetection(cap_height,cap_width,img,mask1_x,mask2_x,mask1_y,mask2_y))
            # value = np.array(SealDetection(cap_height,cap_width,img))
            detect = False
            for v in value:
                if  float(OpenFace_result[frame_count][53]) <= v[0] <= float(OpenFace_result[frame_count][59]) and \
                    ...
                    if frame_count !=1 and detect == True:
                        seal_x,seal_y = CloseLength(seal[frame_count -1][0],seal[frame_count -1][1],seal_x,seal_y,v[0],v[1])
...
def SealDetection(cap_height,cap_width,img,mask1_x,mask2_x,mask1_y,mask2_y):
    ...
    #10,48番マーカーで矩形検出範囲
    cv2.rectangle(mask, (mask1_x,mask1_y), (mask2_x,mask2_y), (255, 255, 255), -1)
    ...
    # エッジ検出 Canny法で二値化
    edges = cv2.Canny(blur_img, 10, 150)
    # 確率的ハフ変換で直線の検出 threshold 10~20
    lines = cv2.HoughLinesP(edges, 1, np.pi / 180, threshold=15, minLineLength=10, maxLineGap=20)
