...
npy_paths = glob.glob(os.path.join(root_dir,"*e/XYZ_localdata.npy"))
...
for i, npy_path in enumerate(npy_paths):
    ...
    #view_initで視点 0,0(または0,180)でyz平面   0,90でxy平面
    # elev,azim = 0,0
    elev,azim = 0,180
    ...
    # アニメーションを保存する
    anim.save(save_dir + f'/3d_animation_sagittal.mp4', fps=30)

    print(f"{i+1}/{len(npy_paths)} anim is saved in {save_dir + f'/3d_animation_sagittal.mp4'}")