...
for progress, bagfile in enumerate(bag_files):
    ...
    # IMMUpath = os.path.dirname(bagfile) + '/' + os.path.basename(bagfile).split('.')[0]
    pipeline = rs.pipeline()
    config = rs.config()
    config.enable_device_from_file(bagfile)
...
try:
    ...
    with open(csv_filename, 'w', newline='') as csvfile:
        fieldnames = ['fps_count', 'accel_x', 'accel_y', 'accel_z', 'norm']
        writer = csv.DictWriter(csvfile, fieldnames=fieldnames)
        writer.writeheader()

        while True:
            # print(f"fps = {fps_count}, pre = {pre_time}")
            frames = pipeline.wait_for_frames()
            cur_time = playback.get_position()  #再生時間の取得 単位はナノ秒
            ...
            if cur_time <= timestamp_RBG[fps_count-1]:  #加速度センサーのfpsが約62.5のためカメラのfps30と合わせる
                ...
                writer.writerow({'fps_count': fps_count, 'accel_x': accel[0], 'accel_y': accel[1], 'accel_z': accel[2], 'norm': np.linalg.norm(accel)})
                pass
