...
seal_temp = "C:/Users/zutom/BRLAB/tooth/Temporomandibular_movement/seal_template/seal_1_trans.png"
    ...
    for i,RGB_dir in enumerate(RGB_dirs):
        ...
        try:
            ...
            while True:
                ...
                seal_position, imgcopy = SealDetection(height, width, imgcopy, mask1_x, mask2_x, mask1_y, mask2_y ,frame_count)
...
def SealDetection(height,width,imgcopy,mask1_x,mask2_x,mask1_y,mask2_y ,frame_count):
    ...
    canny_img = cv2.Canny(blur_img, 10, 150)
    temp = cv2.imread(seal_temp)
    # blur_temp = cv2.GaussianBlur(temp, (5,5), 0)
    # canny_temp = cv2.Canny(blur_temp, 1, 5)

    global best_ratio, best_rot_angle
    # global best_rot_angle, ratio
    global max_Val ; 0
    global rot_temp
    ...
    if frame_count == 1:  #テンプレートマッチングする時の最適な回転量、縮尺を算出
        ...
        for i in range(0, 180+1, 5):
            rot_temp = rotate_template(temp, i)
            blur_temp = cv2.GaussianBlur(rot_temp, (5,5), 0)
            canny_rot_temp = cv2.Canny(blur_temp, 1, 5)
            h, w = canny_rot_temp.shape[0:2]
            # template matching
            result = cv2.matchTemplate(canny_img, canny_rot_temp, cv2.TM_CCOEFF_NORMED)  #ZNCC
        ...
        rot_temp = rotate_template(temp, best_rot_angle)

        #一番マッチする倍率を計算
        result_list = []
        for i in range(20,50,1):
        # for i in range(50,131,1):
            arrenge_rot_temp = cv2.resize(rot_temp.copy(), None, fx=i/100, fy=i/100, interpolation=cv2.INTER_CUBIC)
            arrenge_canny_temp = cv2.Canny(arrenge_rot_temp, 1, 5)
            h, w = arrenge_canny_temp.shape
            result = cv2.matchTemplate(canny_img, arrenge_canny_temp, cv2.TM_CCOEFF_NORMED)
            # 最も類似度が高い位置を取得する。
            minVal, maxVal, minLoc, maxLoc = cv2.minMaxLoc(result)
            result_list.append([i,maxVal,maxLoc,w,h])
        ...
        arrenge_rot_temp = cv2.resize(rot_temp.copy(), None, fx = best_ratio/100, fy = best_ratio/100, interpolation=cv2.INTER_CUBIC)
        arrenge_canny_temp = cv2.Canny(arrenge_rot_temp, 1, 5)
        h_result, w_result = arrenge_canny_temp.shape
        result = cv2.matchTemplate(canny_img, arrenge_canny_temp, cv2.TM_CCOEFF_NORMED)
        # 最も類似度が高い位置を取得する。
        minVal, maxVal, minLoc, maxLoc = cv2.minMaxLoc(result)
    ...
    if frame_count == 1:
        ...
        cv2.imshow("arrenge" , arrenge_canny_temp)
        cv2.waitKey(0)


    print(f"frame = {frame_count}, maxval = {maxVal}, best_rot_angle = {best_rot_angle}, best_ratio = {best_ratio}")
