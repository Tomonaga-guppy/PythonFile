...
npy_paths = glob.glob(os.path.join(root_dir,"2_n/XYZ_localdata.npy"))
...
for i, npy_path in enumerate(npy_paths):
    ...
    # アニメーションを保存する
    anim.save(save_dir + f'/3d_animation.mp4', fps=30)

    print(f"{i+1}/{len(npy_paths)} anim is saved in {save_dir + f'/3d_animation_local.mp4'}")