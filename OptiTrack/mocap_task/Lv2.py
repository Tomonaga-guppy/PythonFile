"""
解答はあくまで一例なので，ほかの方法で算出できそうであればほかでも試してみてください．
コードが冗長な部分もあるので，必要に応じてリファクタリングしてください．
"""
...
def read_3d_optitrack(csv_path):
    """
    CSVファイルからモーキャプデータを読み込み、指定された関節の座標を抽出して補間する
    補間は3次スプラインを使用
    """
...
def main():
    ...
    for frame_num in full_range:
        ...
        # 相対回転行列をRのオブジェクトに変換（Rは回転行列やオイラー角を簡単に求めることができる）
        ...
        # Rオブジェクトからオイラー角を計算（yzxの順で回転させて角度を計算）
        r_hip_angle = r_hip_angle_rot.as_euler('yzx', degrees=True)
        l_hip_angle = l_hip_angle_rot.as_euler('yzx', degrees=True)
        r_knee_angle = r_knee_angle_rot.as_euler('yzx', degrees=True)
        l_knee_angle = l_knee_angle_rot.as_euler('yzx', degrees=True)
        r_ankle_angle = r_ankle_angle_rot.as_euler('yzx', degrees=True)
        l_ankle_angle = l_ankle_angle_rot.as_euler('yzx', degrees=True)


        angle_list.append([r_hip_angle, l_hip_angle, r_knee_angle, l_knee_angle, r_ankle_angle, l_ankle_angle])

        # # おまけ，各キーポイントと座標系を描画する場合（初めは確認してみるといいかも？）
