...
# =========================================================
# 追加：MP4の指定フレームを読む（変更点）
# =========================================================
def read_frame_at_index(mp4_path: Path, frame_index: int = 0):
    """
    MP4動画から指定したインデックスのフレームを読み込む
    ...
    frame_index : int, default=0
        読み込むフレームのインデックス（0始まり）
    
    Returns:
    --------
    frame : numpy.ndarray or None
        読み込んだフレーム画像、失敗時はNone
    """
    cap = cv2.VideoCapture(str(mp4_path))
    if not cap.isOpened():
        print(f"動画を開けません: {mp4_path}")
        return None
    ...
    if frame_index < 0 or frame_index >= total_frames:
        print(f"警告: フレームインデックス {frame_index} は範囲外です（総フレーム数: {total_frames}）")
        cap.release()
        return None
    ...
    if not ok:
        print(f"フレーム {frame_index} の読み込みに失敗しました")
        return None
    ...
    for sub in subs:
        ...
        for direction in directions:
            ...
            for mp4_path in mp4s:
                ...
                frame0 = read_frame_at_index(mp4_path, frame_index=660) #sub1のsagiにあわせたフレーム番号
                if frame0 is None:
