"""
mocapの計算 
mocapで歩行周期を算出して姿勢推定側で対応するフレームを使用していたときのもの
→姿勢推定を基準の歩行周期で比較するほうが自然なため本スクリプトは使用しなくなった．
"""
...
def plot_timeseries_three_stack(
    out_dir: Path,
    title: str,
    frames: np.ndarray,
    ...
    """
    3段（Hip/Knee/Ankle）で時系列をプロットして保存する
    - 横軸はフレーム（時間ではない）
    """
    fig, axes = plt.subplots(3, 1, figsize=(12, 10), sharex=True)

    for ax, k in zip(axes, keys):
        if k not in angle_df.columns:
            ax.set_title(f"{k} (missing)")
            ax.grid(True)
            continue

        y = angle_df[k].to_numpy(dtype=float)
...
def _intermediate_dir(base_dir):
    d = Path(base_dir) / INTERMEDIATE_DIRNAME
...
def read_3d_optitrack(csv_path, start_frame, end_frame):
    ...
    if rigidbody_exists:
        ...
        # # 以前のMarkerSetとRigidBodyのマッピング 20250811はRILICやLILCなかったので番号違う
        # marker_mapping = {
...
def process_one(csv_path_dir: Path, start_frame=0, end_frame=10000):
    """
    subX/theraY-0/mocap を1つ処理
    """
    csv_paths = list(csv_path_dir.glob("[0-9]*-[0-9]*-[0-9]*.csv"))

    for i, csv_path in enumerate(csv_paths):
        print(f"Processing: {csv_path}")

        try:
            keypoints_mocap, full_range, start_frame, end_frame = read_3d_optitrack(csv_path, start_frame, end_frame)
        ...
        ######################################################
        # 完全におまけ 骨盤座標の確認
        rasi_test = np.array([keypoints_mocap[:, 10, x] for x in range(3)]).T
        lasi_test = np.array([keypoints_mocap[:, 2, x] for x in range(3)]).T
        rpsi_test = np.array([keypoints_mocap[:, 14, x] for x in range(3)]).T
        lpsi_test = np.array([keypoints_mocap[:, 6, x] for x in range(3)]).T
        ...
        # =========================
        # Angle timeseries plots (100Hz) - 3段（Hip/Knee/Ankle）
        # =========================
        try:
        ...
        # Flex/Ext (Hip/Knee) + Ankle Pl/Do
        plot_timeseries_three_stack(
            out_dir=csv_path_dir,
            title=f"Timeseries Fl/Ex (Right) - {csv_path.stem}",
            frames=frames_for_plot,
            angle_df=angle_df,
            keys=["R_Hip_FlEx", "R_Knee_FlEx", "R_Ankle_PlDo"],
            fname=f"timeseries_FlEx_R_{csv_path.stem}.png",
        )
        plot_timeseries_three_stack(
            out_dir=csv_path_dir,
            title=f"Timeseries Fl/Ex (Left) - {csv_path.stem}",
            frames=frames_for_plot,
            angle_df=angle_df,
            keys=["L_Hip_FlEx", "L_Knee_FlEx", "L_Ankle_PlDo"],
            fname=f"timeseries_FlEx_L_{csv_path.stem}.png",
        )

        # In/Ex
        plot_timeseries_three_stack(
            out_dir=csv_path_dir,
            title=f"Timeseries In/Ex (Right) - {csv_path.stem}",
            frames=frames_for_plot,
            angle_df=angle_df,
            keys=["R_Hip_InEx", "R_Knee_InEx", "R_Ankle_InEx"],
            fname=f"timeseries_InEx_R_{csv_path.stem}.png",
        )
        plot_timeseries_three_stack(
            out_dir=csv_path_dir,
            title=f"Timeseries In/Ex (Left) - {csv_path.stem}",
            frames=frames_for_plot,
            angle_df=angle_df,
            keys=["L_Hip_InEx", "L_Knee_InEx", "L_Ankle_InEx"],
            fname=f"timeseries_InEx_L_{csv_path.stem}.png",
        )

        # Ad/Ab
        plot_timeseries_three_stack(
            out_dir=csv_path_dir,
            title=f"Timeseries Ad/Ab (Right) - {csv_path.stem}",
            frames=frames_for_plot,
            angle_df=angle_df,
            keys=["R_Hip_AdAb", "R_Knee_AdAb", "R_Ankle_AdAb"],
            fname=f"timeseries_AdAb_R_{csv_path.stem}.png",
        )
        plot_timeseries_three_stack(
            out_dir=csv_path_dir,
            title=f"Timeseries Ad/Ab (Left) - {csv_path.stem}",
            frames=frames_for_plot,
            angle_df=angle_df,
            keys=["L_Hip_AdAb", "L_Knee_AdAb", "L_Ankle_AdAb"],
            fname=f"timeseries_AdAb_L_{csv_path.stem}.png",
        )
        ...
        # ===== 追加: 比較用中間の保存（出力には影響しない） =====
        base_point = int(0)
        hip_z_opti = (((rasi + lasi) / 2 + (rpsi + lpsi) / 2) / 2)[:, 2]
        mid = _intermediate_dir(csv_path_dir) / f"{csv_path.stem}_opti_intermediate.npz"
        np.savez(
            mid,
            start_frame=int(start_frame),
            end_frame=int(end_frame),
            gait_cycles_r_abs=np.array(gait_cycles_r_abs, dtype=float),
            gait_cycles_l_abs=np.array(gait_cycles_l_abs, dtype=float),
            # 正規化平均（比較プロットに使用）
            mean_cycle_r_cols=np.array(mean_cycle_r.columns.tolist(), dtype=object),
            mean_cycle_l_cols=np.array(mean_cycle_l.columns.tolist(), dtype=object),
            mean_cycle_r=mean_cycle_r.to_numpy(dtype=float),
            mean_cycle_l=mean_cycle_l.to_numpy(dtype=float),
        )
        print(f"[OPTI] saved intermediate: {mid}")
...
def main():
    ...
    for sub_i in range(1, 11):
        ...
        try:
            process_one(csv_path_dir, start_frame=0, end_frame=10000)
        except Exception as e:
