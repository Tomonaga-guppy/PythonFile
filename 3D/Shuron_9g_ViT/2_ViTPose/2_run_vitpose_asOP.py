"""
ViTPose（easy_ViTPose）で OpenPoseと互換性のあるJSONを出力するスクリプト

探索:
    root_dir/sub*/thera*/gopro/(fl|fr|sagi)/undistorted_facemasked(undistorted) を対象

実行条件:
    flの場合は undistorted_facemasked を使用して推論
    fr,sagiの場合は undistorted を使用して推論
...
# easy_ViTPose coco_25 → OpenPose BODY_25　キーポイントの対応順が違うため変換用マップ
ORDER_MAP = [
...
def list_thera_dirs(subject_dir: Path):
    ...
    if EXCLUDE_THERA0_1:
        """
        thera0-0_0, thera0-0-1, theraA-0以外を除外
        """
        # thera0-1*, thera0-2* を除外
        thera = [d for d in thera if not d.name.startswith("thera0-1")]
        thera = [d for d in thera if not d.name.startswith("thera0-2")]
