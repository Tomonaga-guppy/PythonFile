...
def _detect_one_image(args):
    """
    ワーカー側（別プロセス）で1枚だけ処理。
    精度に影響する処理は detect_chessboard_corners をそのまま呼ぶだけ。
    """
    img_path_str, checker_pattern, square_size = args

    # OpenCVの内部スレッドとプロセス並列がぶつかると遅くなることがあるので抑制（精度は変わらない）
    try:
        import cv2
        cv2.setNumThreads(1)
    except Exception:
    ...
    if img is None:
        return (img_path_str, False, None, None)  # 読めない
    ...
    if not ret:
        return (img_path_str, False, None, None)

    # corners2, objp, actual_pattern はpickle可能（numpy配列）
    return (img_path_str, True, corners2, (objp, actual_pattern))
...
def main():
    ...
    for direction in directions:
        ...
        result_file = root_dir / direction / "20250811_fault" / "camera_params.json"
        if result_file.exists():
            print(f"警告: 既に '{result_file}' が存在します。続行しますか？ (y/n): ", end="")
            user_input = input().strip().lower()
            if user_input != 'y':
                print("処理を中止しました。")
                continue

        cali_img_folder = root_dir / direction / "20250811_fault" / "cali_imgs"
        cali_imgs = sorted(list(cali_img_folder.glob("*.png")))
        cali_imgs = [p for p in cali_imgs if not p.name.startswith("excluded_")]

        # successフォルダの準備
        success_folder = root_dir / direction / "20250811_fault" / "success"
        ...
        # ワーカーに渡す引数を準備
        tasks = [(str(p), checker_pattern, square_size) for p in cali_imgs]

        # CPU数に応じて
        max_workers = 8

        with ProcessPoolExecutor(max_workers=max_workers) as ex:
            futures = [ex.submit(_detect_one_image, t) for t in tasks]

            for fut in tqdm(as_completed(futures), total=len(futures), desc=f"[{direction}] コーナー検出中"):
                img_path_str, ok, corners2, payload = fut.result()
                cali_img_path = Path(img_path_str)

                if not ok:
                    print(f"  失敗: {cali_img_path.name} - チェッカーボードが検出できません")
                    continue

                objp, actual_pattern = payload
                ...
                corner_img = cv2.drawChessboardCorners(
                    img.copy(), actual_pattern, corners2, True
                )
        ...
        # グラフの生成
        graph_path = root_dir / direction / "20250811_fault" / f"reprojection_errors_{direction}.png"
        ...
        for i, (_, path, _) in enumerate(error_data):
            if path.name.startswith("cali"):
                cali_indices.append(i)
            elif path.name.startswith("near"):
                near_indices.append(i)

        # エラー値（infは0に）
        graph_errors = [item[0] if item[0] != float('inf') else 0 for item in error_data]
        colors = ['red' if item[0] == float('inf') else 'blue' for item in error_data]

        # -----------------------------
        # サブプロット作成
        # -----------------------------
        fig, (ax_cali, ax_near) = plt.subplots(
            1, 2, figsize=(20, 8), sharey=False
        )

        # ===== cali =====
        ax_cali.bar(
            cali_indices,
            [graph_errors[i] for i in cali_indices],
            color=[colors[i] for i in cali_indices]
        )
        ax_cali.set_title("Calibration images (cali)")
        ax_cali.set_ylabel("Reprojection Error (pixels)")
        ax_cali.set_ylim(0, 1)
        ax_cali.set_xticks(cali_indices)
        ax_cali.set_xticklabels(
            [graph_labels[i] for i in cali_indices],
            rotation=90
        )
        ...
        ax_near.bar(
            near_indices,
            [graph_errors[i] for i in near_indices],
            color=[colors[i] for i in near_indices]
        )
        ax_near.set_title("Near images (near)")
        ax_near.set_ylim(0, 7)  # ← near用に狭く
        ax_near.set_xticks(near_indices)
        ax_near.set_xticklabels(
            [graph_labels[i] for i in near_indices],
            rotation=90
        )
        ...
        # 全体調整
        # -----------------------------
        fig.suptitle(f"Per-Image Reprojection Errors for Camera '{direction}'")
