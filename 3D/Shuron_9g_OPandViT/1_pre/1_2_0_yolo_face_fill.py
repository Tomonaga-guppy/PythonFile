"""
PAPT以外の余分な人物の顔を黒塗りするスクリプト
現状FRのみ実施
なお現状, ViTPoseでの推論を行う場合は推論後に余分な人物をはじくようにしているため、このスクリプトの実行は不要
検出人数の設定を行っているOpenPose用に残しているだけ
"""
...
SKIP_IF_OUTPUT_EXISTS = True  # 出力dirが既に存在する場合はスキップするかどうか
EXPAND_SCALE = 2.0  # 黒塗り領域の拡大倍率
