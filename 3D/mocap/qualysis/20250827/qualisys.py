...
def plot_interpolation_results(dfs, labels, marker_name, output_path):
    ...
    original_df, step1_df, step2_df, step3_df = dfs
...
def main():
    for itsv, tsv_file in enumerate(tsv_files):
        ...
        print("\n--- 補間結果のプロットを開始 ---")
        plot_dfs = [nan_df, interpolated_df, interpolated2_df, interpolated3_df]
        plot_labels = ['Original Data (with Gaps)', 'Step 1: Spline Interpolation', 'Step 2: Rigid Body Fitting', 'Step 3: Final Spline Interpolation']
