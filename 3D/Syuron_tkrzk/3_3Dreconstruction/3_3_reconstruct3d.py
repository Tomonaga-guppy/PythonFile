...
def collect_pairs_for_thera(thera_dir: Path):
    ...
    common = sorted(set(fl_methods) & set(fr_methods) & set(sagi_methods), key=lambda x: natural_sort_key(Path(x)))
    return [(m, fl_methods[m], fr_methods[m], sagi_methods[m]) for m in common]
...
def load_projection_matrices_for_subject(sub_dir: Path):
    ...
    P3 = create_projection_matrix(params_sagi)
