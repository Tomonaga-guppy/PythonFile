...
def make_complete_params(camera_params, selected_solution, all_solutions, accuracy_metrics,
                         mp4_name: str, frame0_name: str, checker_pattern, square_size):
...
def main():
    ...
    """
    # --- パラメータ設定（必要ならここだけ変更） ---
    root_dir = Path(r"G:\gait_pattern\2025_shuron_tkrzk")  # ★あなたの現状
    int_dir = Path(r"G:\gait_pattern\int_cali\tkrzk")  # ★内部パラメータ位置は据え置き
    ...
    for sub in subs:
        ...
        for direction in directions:
            ...
            mp4s = sorted(list(mp4_dir.glob("*.MP4")), key=natural_sort_key)
            if not mp4s:
            ...
            for vid_idx, mp4_path in enumerate(mp4s):
                ...
                frame0_path = video_out_dir / "origin.png"
        ...
        base_params = make_complete_params(
            ...
            mp4_name=str(sagi_item['mp4_path'].name),
        ...
        base_params['alignment_info'] = {
            ...
            'pair_board': 'A (fl-fr videos index0)',
            'sagi_board': 'B (bridge video index1 + sagi video index0)',
            'pair_videos': {
                'fl': result_by_dir['fl'][0]['mp4_path'].name if len(result_by_dir['fl']) > 0 else None,
                'fr': result_by_dir['fr'][0]['mp4_path'].name if len(result_by_dir['fr']) > 0 else None,
            },
            'bridge_videos': {
                bridge_dir: {
                    'index0': result_by_dir[bridge_dir][0]['mp4_path'].name,
                    'index1': result_by_dir[bridge_dir][1]['mp4_path'].name,
                }
            },
            'sagi_video': result_by_dir['sagi'][0]['mp4_path'].name,
        }
