...
root_dir = Path(r"G:\gait_pattern\20251014_nakazaki_pythontest")
ori_mov_paths = list(root_dir.glob(f"*nfpa_2*.mp4"))
...
for i, ori_mov_path in enumerate(ori_mov_paths):
    ...
    if not original_imgs_dir.exists():
        ...
        print(f"動画を画像に変換しました: {original_imgs_dir}")
    ...
    pre_img_dir = f" --image_dir " + str(original_imgs_dir)  #画像の場所
    fps = f" --write_video_fps 30"
