...
def find_bottom_frame_by_aruco(video_path, start_frame_rel):
    ...
    ay_diff_list = np.diff(vy_diff_list, prepend=vy_diff_list[0] if vy_diff_list.size > 0 else 0)
    ...
    if frame_numbers_list:  # データが記録されている場合のみグラフを作成
        ...
        ax3.plot(frame_numbers_list, ay_diff_list, marker='.', linestyle='-', label='Marker Y-acceleration', color="tab:green")
