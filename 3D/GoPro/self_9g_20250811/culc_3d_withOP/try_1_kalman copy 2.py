...
# ★★★ 変更点 1 ★★★
# 2種類のデータ系列を受け取るように関数シグネチャを変更
def run_kalman_filter_dual_data(
    estimation_data_series, # パラメータ推定に使うデータ系列 (raw_3d由来)
    filtering_data_series,  # フィルタリング対象のデータ系列 (corrected_3d由来)
    initial_value=0.005
):
    """
    パラメータ推定とフィルタリングで異なるデータ系列を使用するKF関数
    """
    # 1. パラメータ推定は「生データ」の履歴で行う
    ...
    # 3. 生データから推定したパラメータを使って、補正済みデータをフィルタリング
    a_tt, _, _, _ = local_level_kf(y_maf_filt, a1, P1, var_eta, var_eps)
    return a_tt

# ★★★ 変更点 2 ★★★
# apply_frame_by_frame_correction内のKF呼び出しを変更
def apply_frame_by_frame_correction(raw_3d, kps1_seq, kps2_seq, P1, P2):
    ...
    for i in tqdm(range(2, num_frames), desc="フレーム処理"):
        ...
        for r_idx, l_idx, name in KEYPOINT_PAIRS:
            ...
            # (両足エラーのロジックは簡略化のため省略、必要に応じて同様に修正)
            elif accel_r > ACCELERATION_THRESHOLD and accel_l > ACCELERATION_THRESHOLD:
                # 以前のロジックをそのまま使用するか、後述の単独エラーと同様に修正
                # (ここでは簡略化のため、両足とも線形予測で補間)
                for axis in range(3):
...
def main():
    ...
    for subject_dir in subject_dirs:
        ...
        for thera_dir in thera_dirs:
            ...
            output_dir = thera_dir / "3d_gait_analysis_frame_by_frame_kalman_dual_data" # 出力先フォルダ名を変更
