...
def update_animation_frame(frame_idx, animation_data, skeleton_plots, text_plot):
...
def create_3d_animation(animation_data, metadata, save_path, view_config):
    ...
    if np.any(~np.isnan(all_points_transformed)):
        ...
        center = np.array([np.nanmean(all_points_transformed[:, 0]), np.nanmean(all_points_transformed[:, 1]), np.nanmean(all_points_transformed[:, 2])])
        max_range = np.nanmax([x_max-x_min, y_max-y_min, z_max-z_min]) * 0.7
    ...
    ax.set_aspect('auto', 'box') # 描画範囲を自動調整
    ...
    ani = animation.FuncAnimation(
        ...
        fargs=(animation_data, skeleton_plots, text_plot), blit=False, interval=1000/60
    )
...
def main():
    ...
    results_folder_name = "3d_gait_analysis_spline_no_delay" # 読み込み元フォルダ名
    output_animation_dir = root_dir / "animations_multi"
    ...
    data_keys_to_animate = ["points_3d_raw", "points_3d_spline", "points_3d_raw_final"]
    ...
    for json_file in tqdm(json_files, desc="ファイル処理"):
        print(f"\n--- ファイル: {json_file.name} ---")
        # if json_file.parent.parent.name != "thera1-0" and json_file.parent.parent.name != "thera1-1":
        #     print(f"作成省略 {json_file.name}")
        #     continue

        for data_key in data_keys_to_animate:
            if data_key == "points_3d_raw_final":
