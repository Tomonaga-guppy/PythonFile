...
def plot_keypoint_timeseries(raw_data, filtered_data, spline_data, filt_data,
                              output_dir, save_path, frame_range=None):
    """
...
def main():
    ...
    for subject_dir in subject_dirs:
        ...
        for thera_dir in thera_dirs:
            ...
            openpose_csv_path1_butter = thera_dir / "fl_" / "openpose_butter.csv"
            openpose_csv_path2_butter = thera_dir / "fr_" / "openpose_butter.csv"
            ...
            kps1_seq_butter, kps2_seq_butter, frames_butter = load_csv_2d_data(
                openpose_csv_path1_butter, openpose_csv_path2_butter
            )
            if not frames_butter:
                print(f"  - スキップ: 共通フレームが見つかりません（バターフィルタ用）。")
                continue
            print(f"kps1_seq_butter shape: {kps1_seq_butter.shape}, kps2_seq_butter shape: {kps2_seq_butter.shape}")
            # 3D座標を計算（バターフィルタ用）
            raw_kp_3d_butter, confidences_3d_butter = calculate_raw_3d_coordinates(
                kps1_seq_butter, kps2_seq_butter, P1, P2
            )
            print(f"raw_kp_3d_butter shape: {raw_kp_3d_butter.shape}")
            # 信頼度による外れ値フィルタリングを適用（バターフィルタ用）
            conf_filt_kp_3d_butter = confidence_filter_keypoints(
                raw_kp_3d_butter,
                confidences_3d_butter,
                conf_threshold=0.7,      # Confidence閾値
            )
            # 欠損値補間とバターワースフィルタ（フィルタリング後のデータに適用、バターフィルタ用）
            spline_kp_3d_butter = spline_interpolate(conf_filt_kp_3d_butter)
            butter_filt_kp_3d_butter = butterworth_filter(spline_kp_3d_butter, BUTTERWORTH_CUTOFF, FRAME_RATE)
            ...
            np.savez(npz_path,
                     ...
                     raw_butter=raw_kp_3d_butter,  # バターフィルタ用
                     conf_filt_butter=conf_filt_kp_3d_butter,
                     spline_butter=spline_kp_3d_butter,
                     butter_filt_butter=butter_filt_kp_3d_butter,
                     conf_butter=confidences_3d_butter,
            )
