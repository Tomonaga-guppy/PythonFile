...
==============================
fr, sagi の2視点から3D三角測量を行い、補間・フィルタリングして npz 保存
...
def collect_pairs_for_thera(thera_dir: Path):
    ...
    if not (fr_root.exists() and sagi_root.exists()):
        return []
    
    # print(f"Collecting CSV pairs in: {thera_dir}")
    # print(f"  fr: {fr_root}")
    # print(f"  sagi: {sagi_root}")
    ...
    sagi_methods = {}
    for d in sorted([p for p in sagi_root.iterdir() if p.is_dir()], key=natural_sort_key):
        c = d / f"{d.name}{CSV_SUFFIX}"
        if c.exists():
            sagi_methods[d.name] = c

    # 2視点すべてに同じmethodフォルダがあるものだけ採用
    common = sorted(set(fr_methods) & set(sagi_methods), key=lambda x: natural_sort_key(Path(x)))
    return [(m, fr_methods[m], sagi_methods[m]) for m in common]
...
def load_projection_matrices_for_subject(sub_dir: Path):
    ...
    # print(f"Loading extparams from: {fr_dir}, {sagi_dir}")

    fr_json = list(fr_dir.glob(f"{EXT_JSON_PART_NAME}*02.json"))[0]
    sagi_json = list(sagi_dir.glob(f"{EXT_JSON_PART_NAME}*01.json"))[0]

    params_fr = load_camera_parameters(fr_json)
    params_sagi = load_camera_parameters(sagi_json)

    P1 = create_projection_matrix(params_fr)
    P2 = create_projection_matrix(params_sagi)
    
    # print(f"fr_json: {fr_json}")
    # print(f"sagi_json: {sagi_json}")
    return [P1, P2], [fr_json, sagi_json]
...
def _run_one_job(job: Job) -> Tuple[bool, str, float]:
    ...
    try:
        ...
        kps_list, frames = load_csv_2d_data_multi([csv_fr, csv_sagi])
        ...
        out_npz = thera / f"3d_kp_{method_name}_{csv_tag}_fr-sagi.npz"
        np.savez(
            out_npz,
            ...
            meta=np.array(
                [
                    ...
                    f"ext_fr={json_paths[0].name}",
                    f"ext_sagi={json_paths[1].name}",
                ],
                dtype=object,
            ),
        )
        ...
        if SAVE_TIMESERIES_PLOTS:
            ts_dir = thera / f"keypoint_timeseries_3d_{method_name}_{csv_tag}_fr-sagi"
...
def _build_jobs(root_dir: Path) -> List[Job]:
    ...
    for sub in subject_dirs:
        ...
        print(f"thera_dirs in {sub.name}: {[d.name for d in thera_dirs]}")

        for thera in thera_dirs:
            pairs = collect_pairs_for_thera(thera)
            for method_name, csv_fr, csv_sagi in pairs:
                jobs.append(Job(
                    sub_dir=str(sub),
                    thera_dir=str(thera),
