...
# CSV_SUFFIX = "_PT_spline.csv"  # 例: openpose_facemasked_pa_spline.csv
CSV_SUFFIX = "_PA.csv"  # 例: openpose_facemasked_pa_spline.csv
CSV_SUFFIX = "_PT.csv"  # 例: openpose_facemasked_pa_spline.csv
...
def collect_pairs_for_thera(thera_dir: Path):
    ...
    # 3視点すべてに同じmethodフォルダがあるものだけ採用
    common = sorted(set(fl_methods) & set(fr_methods), key=lambda x: natural_sort_key(Path(x)))
    return [(m, fl_methods[m], fr_methods[m]) for m in common]
...
def _run_one_job(job: Job) -> Tuple[bool, str, float]:
    ...
    try:
        ...
        kps_list, frames = load_csv_2d_data_multi([csv_fl, csv_fr])
        ...
        out_npz = thera / f"3d_kp_{method_name}_{csv_tag}_fl-fr.npz"
        np.savez(
            out_npz,
        ...
        if SAVE_TIMESERIES_PLOTS:
            ts_dir = thera / f"keypoint_timeseries_3d_{method_name}_{csv_tag}_fl-fr"
