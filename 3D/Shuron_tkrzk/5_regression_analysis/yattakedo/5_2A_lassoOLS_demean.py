...
# -*- coding: utf-8 -*-
"""
5_2A_lassoOLS_demean.py
======================
regression_dataset.csv + regression_groups.json を入力にして
「患者固定（デミーン）」した上で、LassoCVで変数選択 → 選択変数のみでOLS（statsmodels）を実行する。

- 目的変数: regression_groups.json の PA_gait グループから抽出（このスクリプト内で speed_delta のみに限定）
- 説明変数: PT_assist / PA_basic / PT_basic を組み合わせて複数モデルを回す
- 患者固定: PATIENT_COL（例: pa_id）ごとに、各モデルで使う y と x をデミーン
  （各患者平均との差：x <- x - mean(x|patient), y <- y - mean(y|patient)）
- 毎回、出力はタイムスタンプ付き新規フォルダ（過去結果を参照しない）

出力:
  reg_LassoOLS_grouped_demean/YYYYMMDD_HHMMSS/
    summary_models.csv
...
注意:
- n が小さいため、p値は参考程度（探索的）にしてください。
- LassoCV のCVは「行ベース」の分割です（GroupKFoldはLassoCV.fitにgroupsを渡せないため）。
  ただし患者差はデミーンで取り除いた上で回しています。
"""
...
BASE_OUTDIR = INDIR / "reg_LassoOLS_grouped_demean"
# ===============================================
...
def demean_by_patient(d: pd.DataFrame, cols: list[str], patient_col: str) -> pd.DataFrame:
    """
    d 内で患者ごとに cols をデミーンする。
    NaNは事前に落としてから呼ぶ想定。
    """
    out = d.copy()
    for c in cols:
        if c not in out.columns:
            continue
        out[c] = out[c] - out.groupby(patient_col)[c].transform("mean")
    return out
...
def lasso_then_ols(df: pd.DataFrame, y_col: str, x_cols: list[str], tag: str, outdir: Path):
    ...
    # 欠損除去（このモデルで使う列のみ + patient）
    d = df[[PATIENT_COL, y_col] + x_cols].dropna(axis=0, how="any").reset_index(drop=True)
    n_used = len(d)
    if n_used < MIN_ROWS:
    ...
    # 患者固定（デミーン）
    # ======================
    d = demean_by_patient(d, [y_col] + x_cols, PATIENT_COL)

    # ゼロ分散列は落とす（デミーン後に全て同じになる場合）
    drop_cols = []
    for c in x_cols:
        v = np.nanstd(d[c].to_numpy(float))
        if not np.isfinite(v) or v == 0:
            drop_cols.append(c)
    if drop_cols:
        x_cols = [c for c in x_cols if c not in drop_cols]
        d = d[[PATIENT_COL, y_col] + x_cols].copy()
        if len(x_cols) == 0:
            print(f"SKIP {tag}: all x columns became zero-variance after demeaning")
            return None, None, None
...
def main():
    ...
    if PATIENT_COL not in df.columns:
        raise KeyError(f"{PATIENT_COL} not found in dataset columns. available: {list(df.columns)}")
    ...
    if len(y_list) == 0:
        raise KeyError("PA_gait group is empty in json or columns missing.")
    # 目的変数を speed_delta のみに限定（必要ならここを編集）
