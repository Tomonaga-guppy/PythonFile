"""
ledの発光フレームを基準として、動画の切り出し範囲を選択・保存するスクリプト
"""
...
# 被験者ごとのROI情報を保持する辞書   
roi_cache = {}  # {"fl": (x1, y1, x2, y2), "fr": (x1, y1, x2, y2), "sagi": (x1, y1, x2, y2)}
...
def select_video_range(cap, detected_frame_num, video_path, tpose_flag=False):
    """
    detected_frameを0フレーム目として開始・終了フレームを選択し、動画を切り出す
    既存のJSONファイルがある場合は、相対的な開始・終了フレームを自動適用
    tposeの場合は、開始を0、終了を59フレーム（1秒）に固定する
    """
    # 既存のJSONファイルをチェック
    ...
    need_frame_update = True  # フレーム更新が必要かどうか
    ...
    # トラックバーのコールバック関数
    def on_trackbar(val):
        nonlocal current_relative_frame, is_playing, need_frame_update
        current_relative_frame = val
        is_playing = False
        need_frame_update = True  # トラックバーが動いたらフレーム更新フラグを立てる
    ...
    while True:
        ...
        if need_frame_update:
            ...
            need_frame_update = False  # フレーム更新完了

        # キー入力処理
        wait_time = 1 if is_playing else 30  # 再生中は1ms、停止中は30ms待機
...
def save_trimming_info(video_path, detected_frame_num, start_frame_rel, end_frame_rel, output_video_path):
    ...
    """
    # JSONファイル名を生成
    json_filename = "trimming_info.json"
...
def show_detection_frames(cap, detected_frame_num, roi_box, display_scale, total_frames, total_duration, video_path):
    ...
    output_filename = f"LED_detection_check.jpg"
    output_path = video_path.parent / output_filename
...
def load_roi_cache(sub_dir):
    """
    被験者ディレクトリからROI情報を読み込む
    """
    roi_json_path = sub_dir / "roi_cache.json"
    
    if roi_json_path.exists():
        try:
            with open(roi_json_path, 'r', encoding='utf-8') as f:
                data = json.load(f)
                print(f"ROIキャッシュを読み込みました: {roi_json_path}")
                print(f"保存されているROI情報: {data.get('roi_boxes', {})}")
                return data.get('roi_boxes', {})
        except Exception as e:
            print(f"ROIキャッシュの読み込みに失敗: {e}")
    
    return {}


def save_roi_cache(sub_dir, roi_boxes):
    """
    ROI情報を被験者ディレクトリに保存する
    """
    roi_json_path = sub_dir / "roi_cache.json"
    ...
    data = {
        "metadata": {
            "created_at": datetime.now().isoformat(),
            "subject_directory": str(sub_dir),
            "description": "LED detection ROI coordinates for each direction"
        },
        "roi_boxes": roi_boxes
    }
    ...
    try:
        ...
        print(f"ROIキャッシュを保存しました: {roi_json_path}")
        return True
    except Exception as e:
        print(f"ROIキャッシュの保存に失敗: {e}")
        return False
...
def main():
    ...
    for sub_id in range(15,17): #被験者15〜16まで
        print(f"\n########## 被験者{sub_id} ##########")
        ...
        # 被験者ごとにROIキャッシュをリセット
        global roi_cache
        roi_cache = load_roi_cache(sub_dir)
        
        session_dirs = [d for d in sub_dir.iterdir() if d.is_dir()]
        session_dirs.sort()
        # session_dirsから"cali"を除外
        session_dirs = [d for d in session_dirs if "cali" not in d.name.lower()]
        print(f"セッションディレクトリパス: {session_dirs}")
        print(f"セッションディレクトリ数: {len(session_dirs)}")
        
        for session_dir in session_dirs:
            print(f"\n---- セッションディレクトリ: {session_dir.name} ----")
            tpose_flag = False
            if "thera0-0" in session_dir.name.lower():
                tpose_flag = True #切り取りを3秒間に調整するためのフラグ
            
            skip_frame = 100  # デフォルトは300（時短のため5秒分(適当)飛ばしてから検出）
            # skip_frame = 300  # デフォルトは300（時短のため5秒分(適当)飛ばしてから検出）
            if sub_id == 1 and "thera0-0_0" in session_dir.name.lower():
                skip_frame = 2400  
            elif sub_id == 1 and "thera0-0_1" in session_dir.name.lower():
                skip_frame = 1200
                
            roi_updated_in_session = False  # セッション内でROIが更新されたかどうか
            
            for direction in ["fl", "fr", "sagi"]:
                video_dir = session_dir / "gopro" / direction
                video_path = list(video_dir.glob("GX*.MP4"))[0] #動画は1つしかないので最初のものを取得
                ...
                R_INCREASE_THRESH = 5.0

                # ファイルの存在チェック
                if not video_path.exists():
                    print(f"エラー: 動画ファイルが存在しません: {video_path}")
                    continue  # return → continue に変更
                ...
                # キャッシュされたROIがあるか確認
                if direction in roi_cache:
                    roi_box = roi_cache[direction]
                    print(f"--- ROI情報 ---")
                    print(f"{direction}方向のキャッシュされたROIを使用: {roi_box}")
                else:
                    # ROI選択
                    ...
                    # 選択したROIをキャッシュに保存
                    roi_cache[direction] = roi_box
                    roi_updated_in_session = True
                    print(f"{direction}方向のROIをキャッシュに保存しました")
                ...
                # スキップフレームがある場合、最初のフレームで基準R値を取得 
                if skip_frame is not None and skip_frame > 0:
                    ret, frame = cap.read()
                    if ret:
                        roi_area = frame[orig_y1:orig_y2, orig_x1:orig_x2]
                        if roi_area.size > 0:
                            avg_b, avg_g, prev_avg_r, _ = cv2.mean(roi_area)
                            print(f"スキップ後の開始フレーム({skip_frame})のR値: {prev_avg_r:.1f}")
                    # フレーム位置を戻す
                    cap.set(cv2.CAP_PROP_POS_FRAMES, skip_frame)
                ...
                while cap.isOpened() and processing_window_exists:
                    ...
                    # スキップ情報を表示
                    if skip_frame is not None and skip_frame > 0:
                        text_skip = f"Skipped: {skip_frame} frames"
                        cv2.putText(display_frame, text_skip, (10, 60), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (255, 128, 0), 2)
                ...
                if detected_frame_num is not None:
                    ...
                    # 動画切り出し範囲を選択
                    start_frame_rel, end_frame_rel = select_video_range(cap, detected_frame_num, video_path, tpose_flag)
                ...
                skip_frame = detected_frame_num - 120 # 一度検出されたら他視点のカメラはその120フレーム前から検出を開始する設定
            
        # セッション終了時、ROIが更新されていればJSONに保存
        if roi_updated_in_session:
            save_roi_cache(sub_dir, roi_cache)
            print(f"\nセッション {session_dir.name} 終了後にROI情報を保存しました")
