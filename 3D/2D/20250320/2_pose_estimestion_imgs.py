...
ori_dir_paths = list(root_dir.glob(f"**/Undistort"))
print(f"ori_dir_paths: {ori_dir_paths}")
...
for i, ori_dir_path in enumerate(ori_dir_paths):
    dir_name = ori_dir_path.parent
    print(f"dir_name: {dir_name}")
    print(f"{i+1}/{len(ori_dir_paths)}: {ori_dir_path}")
    stem_name = str(ori_dir_path.stem)
    ...
    pre_img_dir = f" --image_dir " + str(ori_dir_path)  #画像の場所
