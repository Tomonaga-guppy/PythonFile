...
def OpenFace(root_dir):
    pattern = os.path.join(root_dir, '*a1*/RGB_image')
    ...
    for i,RGB_dir in enumerate(RGB_dirs):
        ...
        while True:
            ...
            app.prepare(ctx_id=0, det_size=(640, 640))  #det_sizeとは?

            faces = app.get(img)
            tim = img.copy()
            color = (200, 160, 75)
            for face in faces:
                ins_result = face.landmark_3d_68
                ins_result = np.round(ins_result).astype(int)
                ins_result = (np.array(ins_result))[:, :2]  #68*2
                for i in range(ins_result.shape[0]):
                    f_pos = np.array(ins_result[i])
                    cv2.circle(tim, f_pos, 1, color, 1, cv2.LINE_AA)
            # print(f"f_pos.shape = {ins_result.shape}")
            ins_save_path = dir_path + "/insightface"
            if not os.path.exists(ins_save_path):
                os.mkdir(ins_save_path)
            cv2.imwrite(os.path.join(ins_save_path, "{:04d}.png".format(frame_count)), tim)

            # シールを見つける処理 参照点 48, 54のx, 8のy   シール検出の範囲指定のためintのまま処理
            mask1_x=int(float(ins_result[47,0]))    #48x
            mask1_y=int(float(ins_result[47,1]))    #48y
            mask2_x=int(float(ins_result[54,0]))    #54x
            mask2_y=int(float(ins_result[7,1]))    #8y
            ...
            if key == "scale_seal":
                ...
                xpixel_scale = (float(ins_result[186])-float(ins_result[177]))/(float(ins_result[50])-float(ins_result[41]))
                ypixel_scale = (float(ins_result[242])-float(ins_result[236]))/(float(ins_result[106])-float(ins_result[100]))
                # X68 = X33 + (x68 - x33)*scale
                seal_x = float(ins_result[174]) + (seal_x_pixel - float(ins_result[38]))*xpixel_scale
                seal_y = float(ins_result[242]) + (seal_y_pixel - float(ins_result[106]))*ypixel_scale
            ...
            try:
                for i in range(68):
                    if key == "scale_seal":
                        x = float(ins_result[i,0]) * xpixel_scale
                        y = float(ins_result[i,1]) * ypixel_scale
                    elif key == "scale_openface":
                        x = float(ins_result[i+141]) * xpixel_scale
                        y = float(ins_result[i+209]) * ypixel_scale
                    depthi = img_depth[int(float(ins_result[i,1])),int(float(ins_result[i,0]))] #整数型
