...
def run_imu_on_port(port, barrier, start_queue):
    ...
    try :
        ...
        set_voltage(ser, params=[9,1,0,0]) #1が入力，8がlow出力，9がHigh出力
        ...
        set_voltage(ser, params=[8,1,0,0]) #1が入力，8がlow出力，9がHigh出力
        time.sleep(1)  #1秒間だけLEDが点灯するように
        set_voltage(ser, params=[9,0,0,0]) #1が入力，8がlow出力，9がHigh出力
...
# #9g
            check_port = input(f"""ポート番号の確認
    同期用IMU AP0421533 : {sync_port}
    患者腰用IMU AP0421538 : {sub_port}
    療法士腰用IMU AP0421539 : {thera_port}
